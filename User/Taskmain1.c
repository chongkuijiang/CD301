...
static uint32_t mux=0,min=0xffffffff;
static uint32_t mux2=0,min2=0xffffffff;
void DMA1_Channel1_IRQHandler(void)
{ 	
	if(SYS.AD_Value[0] > mux){
		 mux = SYS.AD_Value[0];
	}
	else if(SYS.AD_Value[0] < min){
		 min = SYS.AD_Value[0];
	}
	
	if(SYS.AD_Value[1] > mux2){
		 mux2 = SYS.AD_Value[1];
	}
	else if(SYS.AD_Value[1] < min2){
		 min2 = SYS.AD_Value[1];
	}
	
  SYS.AD_AddValue[0]+=SYS.AD_Value[0]; //Tem
	SYS.AD_AddValue[1]+=SYS.AD_Value[1];  //EnvTem
...
void GetEnvTem(s16 *data)
{
	...
	dataBuff32s = dataBuff32s - (mux2 + min2);
	dataBuff32s1 -= 2;
	mux2 = 0;
	min2 = 0xffffff;
...
void GetTem(s32 *data)
{
	...
	buffTem = buffTem - (mux + min);
	buffNum -= 2;
	mux = 0;
	min = 0xffffff;
	
	SYS.TemBakAD=buffTem/buffNum;
	SYS.TemBakAD = SYS.TemBakAD*0.8 + last*0.2;
	last = SYS.TemBakAD;
...
void Task1(void) //100ms
{
	...
	else if(CountStep>=100)
	{
		...
		if(i==4)
		{
		//	SYS.TemPID = SYS.TemPID*0.8 + last*0.2;
			e[2]=e[1];
			e[1]=e[0];
			e[0]=SYS.TemPID;
		//	last = SYS.TemPID;
					...
					if(TurnData.step==2) //up1
					{
						...
						if(TurnDataAll.t>=15)
						{
							...
							if(((dataBuff>TurnDataAll.buff))||(TurnData.upDownFlag==0)||(TurnData.upDownFlag==3))
							{
								...
								if(power < 300) {
									SYS.DataSet[P] *= 0.3;
									if(SYS.DataSet[P] > 30) SYS.DataSet[P] = 21;
								}
								else if(power < 700) {
									SYS.DataSet[P] *= 0.5;
									if(SYS.DataSet[P] > 30) SYS.DataSet[P] = 28;
								}
...
//						else if(SYS.TemPV>(SYS.TemSV+1000)) MVresult1=0;
						...
						out = MVresult1 + (ek-lastek)*SYS.DataSet[P]*6;  //6-8
